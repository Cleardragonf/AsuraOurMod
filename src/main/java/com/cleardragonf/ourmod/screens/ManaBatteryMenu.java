...
public class ManaBatteryMenu extends AbstractContainerMenu {
    public final ManaBatteryEntity blockEntity;
    public final Level level;
    private final ContainerData data;

    //Client Constructor
    public ManaBatteryMenu(int containerId, Inventory inv, FriendlyByteBuf extraData){
        this(containerId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(18));
    }

    //Server Constructor
    public ManaBatteryMenu(int containerId, Inventory inv, BlockEntity entity, ContainerData data){
        super(ModMenuTypes.MANA_BATTERY_MENU.get(), containerId);
        checkContainerSize(inv, 1);
        blockEntity = ((ManaBatteryEntity) entity);
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                player, ModBlocks.MANA_BATTERY_BLOCK.get());
    }
